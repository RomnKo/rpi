...
try:
    ...
    drawBlack.text((1, 0), ads_string, font = font24, fill = 0)
    drawBlack.text((1, 30), w_string_today, font = font18, fill = 0)
    drawBlack.text((1, 55), w_string_tomorrow, font = font18, fill = 0)
    drawBlack.text((1, 80), w_string_after_tomorrow, font = font18, fill = 0)
    drawBlack.text((1, 105), time_string, font = font18, fill = 0)

    YellowImage = Image.new('1', (epd.height, epd.width), 255)  # 126*298

    drawYellow = ImageDraw.Draw(YellowImage)
    drawYellow.rectangle((0, 62, 298, 126), fill = (255,255,255,128))

    epd.display(epd.getbuffer(BlackImage), epd.getbuffer(YellowImage))
    time.sleep(10)

    BlackImage2 = Image.new('1', (epd.height, epd.width), 0)
    BlackImage2.paste(white_to_transparency(todays_icon), (15, 15))
    BlackImage2.paste(white_to_transparency(tomorrows_icon), (115, 15))
    BlackImage2.paste(white_to_transparency(day_after_tomorrow_icon), (215, 15))

    drawB2 = ImageDraw.Draw(BlackImage2)
    drawB2.text((20, 95), w_temp_string_today, font = font16, fill = 255)
    drawB2.text((120, 95), w_temp_string_tomorrow, font = font16, fill = 255)
    drawB2.text((220, 95), w_temp_string_after_tomorrow, font = font16, fill = 255)

    YellowImage2 = Image.new('1', (epd.height, epd.width), 255)  # 126*298

    drawY2 = ImageDraw.Draw(YellowImage2)
    drawY2.rectangle((0, 62, 298, 126), fill = 0)

    epd.display(epd.getbuffer(BlackImage2), epd.getbuffer(YellowImage2))

    time.sleep(10)
    YellowImage3 = Image.new('1', (epd.height, epd.width), 255)  # 126*298

    epd.display(epd.getbuffer(BlackImage2), epd.getbuffer(YellowImage3))
